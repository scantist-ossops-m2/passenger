    ...
    ngx_http_core_loc_conf_t *clcf, ngx_str_t *result)
{
    ...
    if (plcf->autogenerated.app_group_name.data == NULL) {
        ...
        (void) ngx_copy(app_root.data, abs_path, app_root.len);
        free(abs_path);
    ...
    PsgJsonValue *server_names_doc)
{
    ...
    ctx->type2 = psg_json_value_begin(server_names_doc, &ctx->it2);
    psg_json_value_end(server_names_doc, &ctx->end2);

    while (!psg_json_value_iterator_eq(ctx->it2, ctx->end2, ctx->type2)) {
        ...
        psg_json_value_iterator_advance(&ctx->it2, ctx->type2);
    }
    ...
    ngx_http_core_loc_conf_t *clcf)
{
    ...
    ctx->type = psg_json_value_begin(loc_configs_container, &ctx->it);
    psg_json_value_end(loc_configs_container, &ctx->end);

    while (!psg_json_value_iterator_eq(ctx->it, ctx->end, ctx->type)) {
        ...
        no_match:
        psg_json_value_iterator_advance(&ctx->it, ctx->type);
    }
...
reverse_manifest_value_hierarchies(manifest_gen_ctx_t *ctx) {
    ...
    ctx->type = psg_json_value_begin(ctx->app_configs_container, &ctx->it);
    ...
    while (!psg_json_value_iterator_eq(ctx->it, ctx->end, ctx->type)) {
        ...
        if (location_configs_container != NULL) {
            ctx->type2 = psg_json_value_begin(location_configs_container, &ctx->it2);
...
reverse_value_hierarchies_in_options_container(PsgJsonValue *options_container,
     PsgJsonValueIterator **it, PsgJsonValueIterator **end, PsgJsonValueType *type)
{
    PsgJsonValue *option_container, *value_hierarchy_doc;
    unsigned int i, len;

    *type = psg_json_value_begin(options_container, it);
    psg_json_value_end(options_container, end);
    while (!psg_json_value_iterator_eq(*it, *end, *type)) {
        ...
        psg_json_value_iterator_advance(it, *type);
    }
}
...
manifest_inherit_application_value_hierarchies(manifest_gen_ctx_t *ctx) {
    ...
    while (!psg_json_value_iterator_eq(ctx->it, ctx->end, ctx->type)) {
        ...
        ctx->type2 = psg_json_value_begin(options_container, &ctx->it2);
        ...
        ctx->type2 = psg_json_value_begin(ctx->default_app_config_container, &ctx->it2);
...
manifest_inherit_location_value_hierarchies(manifest_gen_ctx_t *ctx) {
    ...
    while (!psg_json_value_iterator_eq(ctx->it, ctx->end, ctx->type)) {
        ...
        ctx->type2 = psg_json_value_begin(ctx->default_loc_config_container, &ctx->it2);
        ...
        if (location_configs_container != NULL) {
            ...
            while (!psg_json_value_iterator_eq(ctx->it2, ctx->end2, ctx->type2)) {
                ...
                options_container = psg_json_value_get(location_config_container,
                    "options", -1);
                ctx->type3 = psg_json_value_begin(options_container, &ctx->it3);
                psg_json_value_end(options_container, &ctx->end3);
                while (!psg_json_value_iterator_eq(ctx->it3, ctx->end3, ctx->type3)) {
                    /* For each option, inherit the value hierarchies
                    ...
                    psg_json_value_iterator_advance(&ctx->it3, ctx->type3);
                }
    ...
    PsgJsonValueIterator **it, PsgJsonValueIterator **end, PsgJsonValueType *type)
{
    ...
    for (i = len - 1; i >= 1; i--) {
        ...
        *type = psg_json_value_begin(current_value, it);
    ...
    PsgJsonValueIterator **it, PsgJsonValueIterator **end, PsgJsonValueType *type)
{
    ...
    *type = psg_json_value_begin(doc2, it);
